...
export default function MyMealsPage() {
    ...
    const removePlan = async (date: string, e: React.MouseEvent) => {
        e.stopPropagation(); // Prevent opening modal
        if (!user) return;

        // Optimistic update
        setSavedPlans(prev => prev.filter(p => p.date !== date));
        if (selectedPlan?.date === date) setSelectedPlan(null);
        ...
        if (error) {
            ...
            // Ideally revert state here if failure, but keep simple for now
                ...
                <div className="max-w-4xl mx-auto">
                    <header className="mb-10 flex flex-col md:flex-row justify-between md:items-center gap-4">
                        ...
                        <Link
                            href="/meal-planner"
                            style={{ color: '#ffffff' }}
                            className="bg-[#1a4d3e] hover:opacity-90 !text-white px-6 py-3 rounded-xl font-semibold shadow-lg shadow-[#1a4d3e]/20 dark:shadow-none transition-all text-center"
                        >
                            + New Plan
                        </Link>
                    </header>
                            ...
                            {savedPlans.map((plan) => (
                                ...
                                >
                                            ...
                                            <div className="flex items-center gap-4 text-sm text-gray-500 dark:text-gray-400">
                                                <span className="flex items-center gap-1">
                                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-4 h-4 text-orange-400">
                                                        <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm.75-8.815V4.75a.75.75 0 00-1.5 0v4.435L6.5 12a.75.75 0 00.53 1.28H10a.75.75 0 00.75-.75V9.185z" clipRule="evenodd" />
                                                    </svg>
                                                ...
                                                <span className="flex items-center gap-1">
                                                    ...
                                                    {Object.values(plan.meals).filter(m => m !== null).length} meals
                                                </span>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex items-center gap-3 w-full md:w-auto">
                                        ...
                                        <button
                                            onClick={(e) => removePlan(plan.date, e)}
                        ...
                        >
                            ...
                            >
                                <div className="flex justify-between items-start mb-6 border-b border-gray-100 dark:border-gray-700 pb-4">
                                    <div>
                                        <h3 className="text-2xl font-bold text-gray-900 dark:text-white">
                                            {new Date(selectedPlan.date).toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric' })}
                                        </h3>
                                        <p className="text-[#1a4d3e] dark:text-[#2d6a58] font-medium">
                                            {selectedPlan.totalCalories} kcal Total
                                        </p>
                                    </div>
                                    <button
                                        onClick={() => setSelectedPlan(null)}
                                ...
                                <div className="space-y-4 mb-8 max-h-[60vh] overflow-y-auto pr-2">
                                    {/* Meal Items */}
                                    {Object.entries(selectedPlan.meals).map(([type, meal]: [string, any]) => (
